...
func TestDiagCanResponce(t *testing.T) {
	...
	parser := genxparser.BuildCANMessageParser()
...
func checkCanSensors(sensorsArr []sensors.ISensor, t *testing.T) {
	for _, sens := range sensorsArr {
		...
		case *sensors.FuelSensor:
			{
				assert("Fuel level", sens.(*sensors.FuelSensor).FuelLevel, float32(97.2), t)
				break
			}
		case *sensors.DTCCodesSensor:
			{
				assertStringSlice("DTCCodes", sens.(*sensors.DTCCodesSensor).Codes, []string{"P0128", "P0673", "P0676", "P0675"}, t)
				break
			}
		default:
			{
