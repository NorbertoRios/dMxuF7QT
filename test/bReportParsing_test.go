...
func TestMessageParsing(t *testing.T) {
	...
	file := mock.NewFile("..", "/ReportConfiguration.xml")
	...
	msg := result.(*message.LocationMessage).Messages[0]
	identity := msg.Identity
	if result.(*message.LocationMessage).Ack != "UDPACK 70" {
		t.Error("Failed in ack")
	}
	if identity != "genx_000036002996" {
		t.Error("Failed identity")
	}
	checkBReportSensors(msg.Sensors, t)
}
