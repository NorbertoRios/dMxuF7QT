...
func (parser *GenxBinaryReportParser) Parse(rawMessage *genxmessage.RawMessage) ([]*genxmessage.GenxMessage, string) {
	...
	messages := make([]*genxmessage.GenxMessage, 0)
	...
	for position < len(rawMessage.RawData) && position+firstLen < len(rawMessage.RawData) {
		...
		message := genxmessage.BuildGenxMessage(data, rawMessage.MessageType)
...
func (*GenxBinaryReportParser) readField(message *genxmessage.RawMessage, position int, field *configuration.Field) (interface{}, int) {
	if field.Size == -1 {
		return message.String(position)
	}
	return message.Value(position, field.Size), field.Size
}
